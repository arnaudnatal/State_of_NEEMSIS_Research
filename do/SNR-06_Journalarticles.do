...
count
set graph on
graph combine top bot, col(1) title("Papers published in journals (N=`r(N)')", size(medium)) note("{it:Source:} A. Natal. Updated in September 2025.", size(vsmall)) name(pub, replace)
graph export "Beamer/INPUT/J_global.pdf", as(pdf) replace
